...

{
    ...
    echo "authoritative;"
    echo "option cisco-tftp code 150 = array of ip-address;"
} > "${CONFIG}"
...
for network in $(bashio::config 'networks|keys'); do
    ...
    CISCOTFTP=$(bashio::config "networks[${network}].ciscotftp")
    NEXTSERVER=$(bashio::config "networks[${network}].nextserver")
    {
        ...
        echo "  option cisco-tftp ${CISCOTFTP};"
        echo "  next-server ${NEXTSERVER};"
        echo "}"
    } >> "${CONFIG}"
done
